...
void setup_PPN_VPN_map(void * mem_map,
    ...
    for (uint64_t i = 0; i < BUFFER_SIZE_MB * 1024ULL * 1024ULL; i += HUGE_PAGE_SIZE) {
        uint64_t virt_addr = reinterpret_cast<uint64_t>(mem_map) + i;
...
uint64_t phys_to_virt(uint64_t phys_addr) {
    ...
    uint64_t ppn = phys_addr / HUGE_PAGE_SIZE;
    uint64_t offset = phys_addr % HUGE_PAGE_SIZE;

    if (PPN_VPN_map.find(ppn) == PPN_VPN_map.end()) {
        return 0;
    }

    uint64_t vpn = PPN_VPN_map[ppn];

    return vpn * HUGE_PAGE_SIZE + offset;
}
...
uint64_t measure_bank_latency(volatile char *addr_A, volatile char *addr_B) {
    ...
    volatile char tmp1 = *addr_A;
    volatile char tmp2 = *addr_B;
